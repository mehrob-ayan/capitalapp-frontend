...
export const topupDeposit = (id: number, amount: number, accountId?: number) =>
  request<void>(`/api/v1/assets/${id}/topup`, { method: 'POST', body: JSON.stringify({ amount, accountId: accountId ?? null }) })
...
export interface EffMonth { month: string; growth: number; share: number; capShare: number }
export interface Efficiency {
  ...
  debtLoadPct: number
  interestPaidMonthly: number
  interestEarnedMonthly: number
  netInterestMonthly: number
  liquid: number
  monthlyBurn: number
  runwayMonths: number
  leverage: number
...
export const repayLent = (lentId: number, p: { amount: number; accountId: number; date?: string }) =>
